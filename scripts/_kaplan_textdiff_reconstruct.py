# -*- coding: utf-8 -*-
"""Reconstruct the 633 Kaplan 'text_diff' candidates (correct_answer already
matches, but stem/option/explanation text differs from the PDF) using the
same word-bag-verified extraction approach as the field-bleed and compound
answer_diff passes. Explanation is included this time (the largest single
sub-bucket, 191 items, is 'options match, only explanation differs' -- the
known 'explanation swapped between questions' bug pattern from sessions
44-46, never swept at this scale before).
"""
import json, re, sys
...
def main():
    ...
    Path("scripts/_kaplan_textdiff_report.json").write_text(
