...
def load_cache_dir(d):
    """Return list of {file, n, stem, A, B, C, correct, expl} merging Q+A by n within each file."""
    entries = []
...
def best_match(q, cache_entries):
    ...
    if not qw or sum(qw.values()) < 4:
        return None, 0.0  # stem too short/generic to match reliably on words alone
    ...
    if best is None:
        return None, 0.0
    # Gate: require all 3 options to closely match the cache entry's options (order-sensitive)
    opt_ratios = [
    ...
    if min(opt_ratios) < 0.8:
        return None, 0.0  # stem looked similar but options don't match -> false positive, reject
...
def main():
    ...
    all_entries = []
    all_entries.extend(load_cache_dir("scripts/_cache_cfaweb_qb"))
    all_entries.extend(load_cache_dir("scripts/_cache_cfaweb_mocks"))
    ...
    print()
    Path("scripts/_cfaweb_audit_report.json").write_text(
